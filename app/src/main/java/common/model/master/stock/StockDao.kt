...
abstract class StockDao : DaoGenericOtCompany<MasterStock>() {
    //region paging
    @Query("SELECT * FROM ${MasterStock.ENTITY_NAME} ORDER BY itemCode, batchCode")
    abstract override fun viewAllPaging(): DataSource.Factory<Int, MasterStock>

    @Query("SELECT * FROM ${MasterStock.ENTITY_NAME} WHERE idCompany = :idRelation ORDER BY itemCode, batchCode")
    ...
    abstract fun getNearExpired(idRelation: Int): List<MasterStock>?
    //endregion
}